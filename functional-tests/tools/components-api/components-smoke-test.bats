...
@test "config generate with no existing jobs works and is loadable" {
    ...
    run toolforge components config create "$BATS_FILE_TMPDIR"/generated-config.yaml
    assert_success
}
...
@test "config generate with some existing jobs works, is loadable and creates the same job (slow)" {
    ...
    run toolforge components config create "$BATS_FILE_TMPDIR"/existing-jobs-generated-config.yaml
    assert_success
